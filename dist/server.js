...
export function buildServer() {
    ...
    server.registerTool('get-skills', {
    ...
    server.registerTool('get-education', { description: 'Get Priya Gautam\'s education details.' }, async () => ({
        content: [{
                type: 'text',
                text: assistantProfile.education.map(item => `${item.degree} - ${item.institution} (${item.duration})`).join('\n')
            }]
    }));
    server.registerTool('get-experience', { description: 'Get Priya Gautam\'s professional experience.' }, async () => ({
        content: [{
                type: 'text',
    ...
    server.registerTool('list-sections', {
    ...
    server.registerTool('search-priya-profile', {
    ...
    server.registerTool('search-profile', {
    ...
    server.registerTool('explain-dcr', {
    ...
    server.registerPrompt('introduce-assistant', {
    ...
    server.registerPrompt('answer-about-me', {
...
export async function runHttp(port = Number(process.env.PORT ?? '3000'), host = process.env.HOST ?? '0.0.0.0') {
